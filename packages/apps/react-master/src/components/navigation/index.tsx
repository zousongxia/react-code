				...
				<NavLink
					...
					className={({ isActive }: NavLinkRenderProps) => {
						return (
							...
							(isActive ||
							(getParentPath() === item.path && isFristLevel)
...
const Navigation = ({ className, hide }: Props) => {
	return (
		<div
			className={
				" bg-white w-screen shadow-lg overflow-hidden h-14" + className
			}
		>
			<div className=" max-6xl mx-auto my-0 flex justify-center w-full">
				<div
					className={`w-full flex flex-col relative justify-between items-center min-w-max transition-all duration-300 ${hide ? "top-0" : "-top-14"}`}
				>
					<div className="w-full flex justify-between items-center min-w-max h-14">
